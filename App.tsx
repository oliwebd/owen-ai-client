...
import { Message, Role, OllamaConfig, DEFAULT_HOST, DEFAULT_MODEL, ChatSession, Agent } from './types';
import { MessageBubble } from './components/MessageBubble';
import { SendIcon, StopIcon, SettingsIcon, TrashIcon, SunIcon, MoonIcon, MenuIcon, PlusIcon } from './components/Icons';
...
function App() {
  ...
  // Initialize Connection, History, and Agents
  useEffect(() => {
  ...
  const loadAgents = () => {
    ...
    // If no system prompt is set, and we have a default agent, maybe set it?
    // For now, we leave it empty unless user selects one.
  };
  ...
  const saveCurrentSession = async (currentMessages: Message[]) => {
    // Generate a title if it's a new chat (based on first user message)
    let title = 'New Chat';
    ...
    // Update state to reflect saved ID (if it was new)
    if (!currentSessionId) {
  ...
  const handleSendMessage = async () => {
    ...
    // Save immediately so user query is recorded
    ...
    // We keep a temporary state for UI that includes the empty assistant bubble
    ...
    try {
      ...
      // Stream finished, save the full interaction to DB
